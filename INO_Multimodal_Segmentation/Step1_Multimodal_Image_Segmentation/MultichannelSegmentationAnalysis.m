%Script to show concentration mCerulean3 and Venus in cell images from INO
clc; clear all; close all;


%%%%%%%% USER DEFINED PARAMETERS
%Path to all aqcuired data
Path= 'D:\REPS_qF3 FLIM-FRET + DRAQ5 and Mitotracker RED\20190626_Fun for Nehad rep1_compressed\All data';

%Well that includes untransfected mCerulean3-protein cells to extract
%unbound lifetime 
donor_filename = 'WellID_E2-20190627T171341.658-vol0.tif'; 
donor_fullpath = fullfile(Path,donor_filename); 
%Get FLIM and Spectral Cubes 
donor_flim  = imread(donor_fullpath,1); 
donor_spec = imread(donor_fullpath,2); 
donor_ven_img = squeeze(sum(donor_spec(:,:,13:19),3));
...
global_donor_decay = global_donor_decay(:,62:462); 
[G_donor,S_donor]= GetINOPhasor(global_donor_decay');

%Let's Read FLIM and Spectral Cubes for both configuration
filename= 'WellID_E6-20190627T204903.931-vol0.tif';
filename2 = 'WellID_E6-20190627T204945.682-vol0.tif';
...
mc3_standard_slope =  0.1525; 
ven_standard_slope = 4.8721;
...
figure; 
subplot(2,2,1); imagesc(mc3_img); 
axis image off ;
subplot(2,2,2); imagesc(ven_img); 
axis image off ;
subplot(2,2,3); imagesc(mitotracker_img); 
axis image off ;
subplot(2,2,4); imagesc(draq5_img); 
axis image off ;
colormap('gray'); 
%Using defined experimental parameters, let's get intensity images
%These are the sum of the FLIM/HS cubes using start and end indecies



 

% spectral intensity range for total DRAQ5 Intensity
draq5_low_threshold = 250 ; 
draq5_high_threshold = 3500 ; 
mito_low_threshold = 300; 
...
sub_mitomap_roi_decays = mitomap_roi_decays(:,62:462); 
sub_mc3map_roi_decays = mc3map_roi_decays(:,62:462); 
sub_draq5_roi_decays = draq5_roi_decays(:,62:462); 
...
acceptor_to_donor_img = double(ven_img) ./ double(mc3_img); 
acceptor_to_donor_img(ven_img<100) =0 ;
acceptor_to_donor_img(mc3_img < 5) = 0 ;
%for each map we will calculate mean measurements and roi size

[mito_roi_size,mito_ratio] = GetMeanMeasurementsUsingMaps(mitomap,acceptor_to_donor_img); 
[mc3_roi_size,mc3_ratio] = GetMeanMeasurementsUsingMaps(mc3map, acceptor_to_donor_img); 
[draq5_roi_size,draq5_ratio] = GetMeanMeasurementsUsingMaps(draq5map, acceptor_to_donor_img); 
...
WriteResultsToExcel(mito_results_path,mito_roi_ids,mito_roi_size',mito_ratio,mito_lifetime,mito_fret);
WriteResultsToExcel(mc3_results_path,mc3_roi_ids,mc3_roi_size',mc3_ratio,mc3_lifetime,mc3_fret);
WriteResultsToExcel(draq5_results_path,draq5_roi_ids,draq5_roi_size',draq5_ratio,draq5_lifetime,draq5_fret);
