...
log_size = 50;          % Specifiy size of log filter
...
% figure()
% imshow(im(ni).BW1)          
               [im(ni).L, im(ni).num] = watershed_segmentation(im(ni).double_img2(7:end-6,7:end-6), im(ni).BW1);
...
%                 text(im(ni).newstats2(news).Extrema(8,1),im(ni).newstats2(news).Extrema(8,2),num2str(news),'BackgroundColor','w','color','r','Fontsize',6)
                %text(im(ni).newstats2(news).Extrema(8,1),im(ni).newstats2(news).Extrema(8,2),num2str(news),'color','r','Fontsize',8)
            end
            hold off
